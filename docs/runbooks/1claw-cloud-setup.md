...
- **Claim polling:** the background worker calls **`GET /v1/platform/apps/{app_id}/users`** once per poll tick (Bearer `plt_`), matches each pending user’s `connection_id` to the returned rows, and treats non-empty **`claimed_at`** (or permissive **`status`** heuristics) as “claim complete”. Canonical upstream docs: [Platform API guide](https://docs.1claw.xyz/docs/guides/platform-api). Prefer this over undocumented `GET /v1/platform/connections/{connection_id}` probes. Until you have a webhook, this list endpoint is the supported operator-visible signal path.
