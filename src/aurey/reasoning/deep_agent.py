...
AUREY_DEEP_USER_PROMPT = (
    ...
    "- Aurey peer sends: in the **same turn** as prepare/execute, call "
    "``resolve_hosted_recipient_by_handle`` before ``tx_prepare_*`` (do not reuse a bare ``0x`` "
    "from memory). Confirm using the handle. On ``recipient_not_found`` or ``recipient_wallet_unavailable``, "
