...
AUREY_DEEP_USER_PROMPT = (
    ...
    "prepare; confirm using the handle. On ``recipient_not_found``, you MUST paste the full "
    "``invite_deeplink`` URL from the tool output (top-level or ``error.invite_deeplink``) for the "
    "user to share—no escrow. When ``resolved_via_handle_claim`` is true, show ``telegram_user_id`` and "
